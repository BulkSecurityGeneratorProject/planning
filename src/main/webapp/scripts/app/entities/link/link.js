'use strict';

angular.module('planningApp')
    .config(function ($stateProvider) {
        $stateProvider
            .state('link', {
                parent: 'entity',
                url: '/links',
                data: {
                    authorities: ['ROLE_USER'],
                    pageTitle: 'Links'
                },
                views: {
                    'content@': {
                        templateUrl: 'scripts/app/entities/link/links.html',
                        controller: 'LinkController'
                    }
                },
                resolve: {
                }
            })
            .state('link.detail', {
                parent: 'entity',
                url: '/link/{id}',
                data: {
                    authorities: ['ROLE_USER'],
                    pageTitle: 'Link'
                },
                views: {
                    'content@': {
                        templateUrl: 'scripts/app/entities/link/link-detail.html',
                        controller: 'LinkDetailController'
                    }
                },
                resolve: {
                    entity: ['$stateParams', 'Link', function($stateParams, Link) {
                        return Link.get({id : $stateParams.id});
                    }]
                }
            })
            .state('link.new', {
                parent: 'link',
                url: '/new',
                data: {
                    authorities: ['ROLE_USER'],
                },
                onEnter: ['$stateParams', '$state', '$uibModal', function($stateParams, $state, $uibModal) {
                    $uibModal.open({
                        templateUrl: 'scripts/app/entities/link/link-dialog.html',
                        controller: 'LinkDialogController',
                        size: 'lg',
                        resolve: {
                            entity: function () {
                                return {
                                    text: null,
                                    link: null,
                                    id: null
                                };
                            }
                        }
                    }).result.then(function(result) {
                        $state.go('link', null, { reload: true });
                    }, function() {
                        $state.go('link');
                    })
                }]
            })
            .state('link.edit', {
                parent: 'link',
                url: '/{id}/edit',
                data: {
                    authorities: ['ROLE_USER'],
                },
                onEnter: ['$stateParams', '$state', '$uibModal', function($stateParams, $state, $uibModal) {
                    $uibModal.open({
                        templateUrl: 'scripts/app/entities/link/link-dialog.html',
                        controller: 'LinkDialogController',
                        size: 'lg',
                        resolve: {
                            entity: ['Link', function(Link) {
                                return Link.get({id : $stateParams.id});
                            }]
                        }
                    }).result.then(function(result) {
                        $state.go('link', null, { reload: true });
                    }, function() {
                        $state.go('^');
                    })
                }]
            })
            .state('link.delete', {
                parent: 'link',
                url: '/{id}/delete',
                data: {
                    authorities: ['ROLE_USER'],
                },
                onEnter: ['$stateParams', '$state', '$uibModal', function($stateParams, $state, $uibModal) {
                    $uibModal.open({
                        templateUrl: 'scripts/app/entities/link/link-delete-dialog.html',
                        controller: 'LinkDeleteController',
                        size: 'md',
                        resolve: {
                            entity: ['Link', function(Link) {
                                return Link.get({id : $stateParams.id});
                            }]
                        }
                    }).result.then(function(result) {
                        $state.go('link', null, { reload: true });
                    }, function() {
                        $state.go('^');
                    })
                }]
            });
    });
